...
function BarChart({ current, previous, period }: { current: PeriodStats; previous: PeriodStats; period: Period }) {
  const bars = period === 'week'
    ? current.dailyBreakdown.map((d, i) => ({ label: DAY_LABELS[i], value: d.meditationMinutes + d.breathingMinutes, dateKey: d.dateKey }))
    : period === 'month'
    ? Array.from({ length: 4 }, (_, wi) => {
        const weekDays = current.dailyBreakdown.slice(wi * 7, (wi + 1) * 7)
        const value = weekDays.reduce((s, d) => s + d.meditationMinutes + d.breathingMinutes, 0)
        return { label: `Нед ${wi + 1}`, value, dateKey: weekDays[0]?.dateKey ?? '' }
      })
    : Array.from({ length: 12 }, (_, mi) => {
        const monthDays = current.dailyBreakdown.filter(d => parseInt(d.dateKey.split('-')[1]) - 1 === mi)
        const value = monthDays.reduce((s, d) => s + d.meditationMinutes + d.breathingMinutes, 0)
  ...
  return (
    <div className="flex items-end gap-1.5 h-16">
      {bars.map((bar, i) => {
        const height = Math.max((bar.value / maxVal) * 100, bar.value > 0 ? 8 : 3)
          ...
          <div key={i} className="flex flex-col items-center gap-1 flex-1">
            <div
              ...
              style={{
                ...
                boxShadow: isToday && bar.value > 0 ? 'var(--glow-amber)' : 'none',
              }}
            />
            <span className="text-[9px]" style={{ color: 'rgba(255,220,170,0.3)' }}>{bar.label}</span>
          </div>
        )
      })}
    </div>
  ...
  return (
    <div className="flex flex-col gap-3 mt-2 max-w-lg">
      ...
      <p className="text-xs px-1" style={{ color: 'rgba(255,220,170,0.35)' }}>
        Эт{periodLabel === 'неделю' ? 'а' : 'от'} {periodLabel} vs {prevLabel} {periodLabel === 'неделю' ? 'неделя' : periodLabel}
      </p>
      ...
      <div className="grid grid-cols-2 gap-2">
        {stats.map(s => (
          <GlassCard key={s.label} className="p-4">
            ...
            <p className="text-xs mt-0.5" style={{ color: 'rgba(255,220,170,0.3)' }}>{s.sub}</p>
            <p className="text-xs mt-0.5" style={{ color: 'rgba(255,248,235,0.4)' }}>{s.label}</p>
          </GlassCard>
        ))}
      </div>
      ...
      <GlassCard className="p-4">
        ...
        {current.meditationMinutes === 0 && current.breathingSessionCount === 0 && (
          <p className="text-xs text-center mt-3" style={{ color: 'rgba(255,220,170,0.25)' }}>
